...
public final class SelectHandler {
	...
	public static void handle(String stmt, FrontSession session, int offs) {
		int offset = offs;
		...
		case ServerParseSelect.SESSION_INCREMENT:
			SessionIncrement.response(session);
			break;
		case ServerParseSelect.SESSION_ISOLATION:
			SessionIsolation.response(session);
			break;
		case ServerParseSelect.LAST_INSERT_ID:
			...
			SelectLastInsertId.response(session, stmt, offset);
			break;
		case ServerParseSelect.IDENTITY:
			...
			offset = ServerParseSelect.skipAs(stmt, offset);
			SelectIdentity.response(session, stmt, offset, orgName);
			break;
		default:
			session.execute(stmt, ServerParse.SELECT);
		}
	}
