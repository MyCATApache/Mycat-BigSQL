...
abstract class MultiNodeHandler implements ResponseHandler, Terminatable {
	...
	public boolean clearIfSessionClosed(FrontSession session) {
		if (session.closed()) {
	...
	protected ErrorPacket createErrPkg(String errmgs) {
		...
		err.message = StringUtil.encode(errmgs, session.getConInfo()
				.getCharset());
		return err;
	}
	...
	protected void tryErrorFinished(boolean allEnd) {
		if (!errorRepsponsed && allEnd && !session.closed()) {
			...
			session.writeErrorPkg(createErrPkg(this.error));
			...
			if (session.isAutocommit()) {
				...
				session.setTxInterrupt(this.error);
				// clear resouces
				clearResources();
			}
