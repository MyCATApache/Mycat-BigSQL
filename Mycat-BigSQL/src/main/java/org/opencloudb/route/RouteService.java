...
public class RouteService {
	...
	public RouteResultset route(SystemConfig sysconf, SchemaConfig schema,
			int sqlType, String stmt, String charset, FrontSession session)
		...
		if (stmt.startsWith(mycatHint)) {
			...
			if (endPos > 0) {
                ...
                if(firstSplitPos > 0 ){
                    ...
                    if(hintHandler != null){
                        rrs = hintHandler.route(sysconf,schema,sqlType,realSQL,charset,session,tableId2DataNodeCache,hintValue);
                    }else{
			...
			rrs = RouteStrategyFactory.getRouteStrategy().route(sysconf, schema, sqlType, stmt,
					charset, session, tableId2DataNodeCache);
		}
