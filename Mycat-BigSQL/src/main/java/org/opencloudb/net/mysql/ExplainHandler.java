...
public class ExplainHandler {
	...
	public static void handle(String stmt, FrontSession session, int offset) {
		...
		RouteResultset rrs = getRouteResultset(session, stmt);
		if (rrs == null)
			return;
		...
		for (RouteResultsetNode node : rrsn) {
			RowDataPacket row = getRow(node, charset);
	...
	private static RouteResultset getRouteResultset(FrontSession session,
			String stmt) {
		ConnectionInfo conInfo = session.getConInfo();
		String db = conInfo.getSchema();
		if (db == null) {
			session.writeErrMessage(ErrorCode.ER_NO_DB_ERROR,
		...
		SchemaConfig schema = MycatSystem.getInstance().getConfig()
				.getSchemas().get(db);
		if (schema == null) {
			session.writeErrMessage(ErrorCode.ER_BAD_DB_ERROR,
					"Unknown database '" + db + "'");
			return null;
		}
		try {
			...
			return MycatSystem
					...
					.route(MycatSystem.getInstance().getConfig().getSystem(),
							schema, sqlType, stmt, conInfo.getCharset(), null);
		} catch (SQLNonTransientException e) {
			StringBuilder s = new StringBuilder();
			logger.warn(s.append(session.getConInfo()).append(stmt).toString()
					+ " error:" + e);
			String msg = e.getMessage();
			session.writeErrMessage(ErrorCode.ER_PARSE_ERROR, msg == null ? e
