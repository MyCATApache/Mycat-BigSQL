...
public class CharacterSet {
	...
	public static void response(String stmt, FrontSession session, int rs) {
		if (-1 == stmt.indexOf(',')) {
			/* 单个属性 */
	...
	private static void oneSetResponse(String stmt, FrontSession c, int rs) {
	...
	private static void multiSetResponse(String stmt, FrontSession session, int rs) {
		...
		for (int i = 1; i < sqlList.length; i++) {
			...
			default:
				...
				logger.warn(s.append(session).append(sql).append(" is not executed")
						.toString());
			}
		}
		...
		if (charConnection.equalsIgnoreCase(charResult)) {
			...
			session.writeErrMessage(ErrorCode.ER_UNKNOWN_CHARACTER_SET, sb.toString());
		}
	}

	private static void setCharset(String charset, FrontSession session) {
		if ("null".equalsIgnoreCase(charset)) {
			/* 忽略字符集为null的属性设置 */
			session.writeOK();
		} else if (session.getConInfo().setCharset(charset)) {
			session.writeOK();
		} else {
			try {
				if (session.getConInfo().setCharsetIndex(Integer.parseInt(charset))) {
					session.writeOK();
				} else {
