...
public final class ShowHandler {
	...
	public static void handle(String stmt, FrontSession session, int offset) {
		...
		case ServerParseShow.DATABASES:
			ShowDatabases.response(session);
			break;
		case ServerParseShow.TABLES:
			ShowTables.response(session, stmt);
			break;
		case ServerParseShow.MYCAT_STATUS:
			ShowMyCatStatus.response(session);
			break;
		case ServerParseShow.MYCAT_CLUSTER:
			ShowMyCATCluster.response(session);
			break;
		default:
			session.execute(stmt, ServerParse.SHOW);
		}
	}
